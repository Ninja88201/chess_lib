...
mod tests {
    ...
    // static NODES: [i64; 7] = [20, 400, 8902, 197281, 4865609, 119060324, 3195901860];
    
    // Position 2
    // static POSITION: &'static str = "8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1";
    // static NODES: [i64; 6] = [14, 191, 2812, 43238, 674624, 11030083];
    ...
    fn depth_4() {
        ...
        assert_eq!(pos, NODES[3]);
    }
    #[test]
    fn depth_5() {
        let mut board = Board::new_from_fen(POSITION).unwrap();
        let pos = board.positions_divide(5);
        assert_eq!(pos, NODES[4]);
    }
    // #[test]
    // fn depth_6() {
    //     let mut board = Board::new_from_fen(POSITION).unwrap();
