    ...
    pub fn from_algebraic(s: &str) -> Option<Self> {
        ...
        if s == "O-O" || s == "0-0" {
            return Some(Self::new(
                Tile::new_unchecked(4),  // e1 or e8 (to be fixed later)
                Tile::new_unchecked(6),  // g1 or g8
                Piece::King,
                None,
                None,
                CastlingRights::NONE,
            ...
            return Some(Self::new(
                Tile::new_unchecked(4),
                Tile::new_unchecked(2),
                Piece::King,
                None,
                None,
                CastlingRights::NONE,
        ...
        // 1. Parse piece (default to Pawn)
        let piece = if let Some(&c) = chars.peek() {
            match c {
                'N' => { chars.next(); Piece::Knight }
                'B' => { chars.next(); Piece::Bishop }
                'R' => { chars.next(); Piece::Rook }
                'Q' => { chars.next(); Piece::Queen }
                'K' => { chars.next(); Piece::King }
                _ => Piece::Pawn,
            }
        } else {
            return None;
        ...
        // 2. Optional disambiguation skipped here (can't fill `from` without board info)

        // 3. Capture mark 'x'
        if let Some(&'x') = chars.peek() {
            chars.next();
            // Capture flag, but piece captured unknown here
        }

        // 4. Destination square (mandatory)
        ...
        // 6. En passant, capture piece, from, castle, caches - not inferable here, set defaults
        Some(Self::new(
            Tile::new_unchecked(0), // from unknown
            to,
            piece,
            None,                    // capture unknown without board
            None,                    // en passant
            CastlingRights::NONE, // castling rights
            promotion,
            None,
            None,
        ))
