...
async fn main() {
    let mut board = Board::new_from_fen("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1").unwrap();
    ...
    // let rand2 = macroquad::rand::RandGenerator::srand(&self, macroquad::time::get_time() as u64);
    loop {
        ...
        if is_mouse_button_pressed(MouseButton::Left) {
            if let Some(clicked_tile) = get_tile(mouse_position().into(), flipped) {
                match selected_tile {
                    Some(tile) => {
                        let result = board.try_move_piece(
                            tile,
                            clicked_tile,
                            &graphical_promote,
                            (&piece_atlas, board.white_turn, flipped),
                        );
                        match result.await {
                            Ok(_) => {
                                selected_tile = None;
                            ...
                            Err(e) => {
                                ...
                                match e {
                                    ...
                                    me::FriendlyCapture => selected_tile = Some(clicked_tile),
        ...
        if is_key_pressed(KeyCode::Space) {
            let mut moves = MoveList::new();
            board.generate_legal_moves(board.white_turn, &mut moves);
            let _ = board.make_move_unchecked(moves[rand.gen_range(0, moves.len())]);
        }
        ...
        for (i, &p) in Piece::PROMOTION_PIECES.iter().enumerate()
        {
            draw_texture_ex(
                ...
                DrawTextureParams { 
                    ...
                    source: Some(get_piece_sprite_rect(p, context.1)), 
                    ..Default::default()
                }
            );
        }
...
fn render_board(atlas: &Texture2D, board: &mut Board, selected: Option<Tile>, flipped: bool) {
    ...
    for file in 0..8 {
        for rank in 0..8 {
            let (x, y) = tile_to_screen(Tile::new_xy(file, rank).unwrap(), flipped);
            ...
            if white_in_check {
                if Tile::new_xy(file, rank).unwrap()
                    == board.white.bb[Piece::King as usize].to_bit().unwrap()
                {
                    color.r = 1.0;
                    color.g *= 0.5;
                    color.b *= 0.5;
            ...
            if black_in_check {
                if Tile::new_xy(file, rank).unwrap()
                    == board.black.bb[Piece::King as usize].to_bit().unwrap()
                {
                    color.r = 1.0;
                    color.g *= 0.5;
                    color.b *= 0.5;
...
fn get_tile(pos: Vec2, flipped: bool) -> Option<Tile> {
    ...
    match flipped {
        true => {
            ...
            return Tile::new_xy(clicked_file, rank);
        }
        false => {
            let clicked_rank = 7 - rank;
            return Tile::new_xy(file, clicked_rank);
        }
    }
}
